...
- `compset/xianshuihe.gmsh.lite/`: gmsh-meshed (C_mesh=3) Xianshuihe fault (鲜水河断裂), eastern Tibet — **runnable**. 1:1M Chinese active-fault KML parsed by `plot_fault_trace.py` into **7 faults**, ft1..ft7 SE→NW via `MERGE_GROUPS = [[1,2],[0,4],[3],[5],[6],[7],[8]]` (482.7 km total; all vertical, `FT_DIP_DEG=90`, `FT_TYPE=1`). `map_step_overs.py` classifies junction and splay-strand step-overs releasing/restraining with the sinistral convention. Embedded-fault pure-quad mesh (`Mesh.SubdivisionAlgorithm=1`): 54,068 quads, 0 triangles, 0 orphans at dxy=400 m. Per-node loading from **GSRM v2.1** — `fetch_strain_rate.sh` → `strain_rate_loading.py --case` (samples the 2581 mesh fault nodes) → `apply_strain_loading.py --target-stress`. Use **TARGET = 90–100 MPa**, not the SAF's 120: the angle of compression here straddles zero (43% positive) instead of being coherently positive (91% for the SAF), so T/N > 1 drives nodes tensile. Reference: Qiao et al. (2022, EPSL 596, 117799).
